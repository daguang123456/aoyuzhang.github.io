...
if (this.bCallBack || !form || !form.ICAction || !form.ICResubmit || this.id.indexOf('Resubmit') != -1) return true;try {
 ...
 alert("Error in opening/sending request, function loadXMLDoc: - Error: " + err + " - Url:" + this.url + ", - URL Parameters: " + this.params);  }
 }
 }
 }
 catch (e)
...
reduceScrollareaHeight:function(id, data, obj)
 ...
 {
 if ((typeof obj.baseURI != "undefined") && obj.baseURI.indexOf ("ICRCFModalJS=1") != -1 && obj.id.indexOf ("divPAGECONTAINER") != -1) {
...
DoUrlAJAX: function(sURL, scriptList) {
 ...
 if (top.window.isFModeLayout()){
 if (sURL.indexOf('?') == -1)
 sURL += '?ICDoModeless=1'; else
 sURL += '&ICDoModeless=1'; window.location.href = sURL; }
 else {
 if (this.bModal == 1 || this.bModal == 2) 
 window.doModeless(sURL, -1, -1, ''); else
...
/* processing inline stylesheet */
var nStyle=-1;nLen = (net2.ENDSTYLE).length; // length of %/style>
pos=sData.indexOf("%style");while(pos>-1){
