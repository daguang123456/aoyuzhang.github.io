...
function loadExtGrouplet(url, id, nIndex, worker) {
 ...
 el.setAttribute("data-tH", url.substring(xTH+4, xTH+5)); var txtHTML = "<iframe id='" + el.id + "_iframe' class='psc_groupletiframe' name='" + el.id + "_iframe'"; var title = 'Contenu'; if (el.id.indexOf("grouplet") != -1) {
...
function loadGrouplet(url, id, nIndex, worker) {
 ...
 el.innerHTML = ""; var title = 'Contenu'; if (el.id.indexOf("grouplet") != -1) {
 if (el.id.indexOf("grouplet_") != -1)
 var sId = el.id.substring(el.id.indexOf('_') + 1); else
 var sId = el.id.substring(el.id.indexOf('t') + 1); var titleObj = document.getElementById(sId); if (!titleObj) {
 sId = sId.replace('$', '_LBL$'); titleObj = document.getElementById(sId); }
...
function expandAcc(fname, bMaxH, bDefault) {
 ...
 return; }
 removeClose(obj); removeClose(obj.children[1]); setAriaAttr(obj.children[0].children[0], "aria-expanded", "true"); var pos = ptCommonObj2.getPosition(obj); var nMaxH = fullHeight - (pos.y + nHeaderHeight+(nCur+1) * nHeaderHeight); if (arrCont.length == 2 && (isHidden(arrCont[1]) || isHidden(arrCont[0])))
 ...
 if (objOpen) {
 addClose(objOpen); setAriaAttr(objOpen.children[0].children[0], "aria-expanded", "false"); addClose(objOpen.children[1]); var cobj = objOpen.querySelector(".ps_accordion_content"); if (bMaxH) {
...
function DoTabbing(evt, id, bReset, oParentWin) {
 ...
 oWin = oParentWin; var objWrap = oWin.document.getElementById("PT_WRAPPER"); var isShiftTab = isShiftKey(oLastKeyEvent) && getKeyCode(oLastKeyEvent) == 9 ? true : false; if (objWrap)
 {
 ...
 if (!target || (objWrap && target.id && target.id.indexOf("ICLastAnchor") != -1) && isTab) {
...
function FileAttachment(fList, dropArea, field, uploadBT, clistBT, numFiles) {
 ...
 var p = document.createElement("p"); var pText = document.createTextNode("Taille fichier: " + fsize + " Bytes"); if(fsize != "") p.appendChild(pText);  li.appendChild(p); fileList.appendChild(li); cloudFileQueue.push({ name: fname, url: furl, li: li , authtoken: pauthtoken }); enableControl(uploadBT, true, hrefUpload); replaceClass(wrapper, "psc_attach-selecting", "psc_attach-selected"); enableControl(clistBT, true, hrefClear); if (numFilesAllowed <= fileList.childElementCount ) {
 DisableAllFileAdd(); }
 ...
 "Taille fichier: " + Math.round(file.size / 1024) + "KB"
 ...
 clearInterval(intervalId); replaceClass(wrapper, "psc_attach-uploading", "psc_attach-completed"); var div = document.getElementById("divIndicater"); div.style["width"] = "100%"; psAnnounceText("Chargement termin\u00e9"); enableControl(document.getElementById("#ICCancel"), false); okBT.focus(); }, false); upload.addEventListener("error", function (ev) { console.log(ev); }, false);  var pUrl = eval("postUrl_" + fileField.form.name); xhr.open("POST", pUrl); if (typeof xhr.withCredentials != 'undefined')
...
var PTNavBar = {
 ...
 PTNavBar.SetAnnounceText("Les boutons Retour vers parent et Retour vers racine peuvent \u00eatre utilis\u00e9s."); if (backMenuPrevEl != null && PTNavBar.backMenuEl == null)
 PTNavBar.SetAnnounceText("Les boutons Retour vers parent et Retour vers racine ne peuvent plus \u00eatre utilis\u00e9s."); clearTimeout(t2);},2000);  }
 },
