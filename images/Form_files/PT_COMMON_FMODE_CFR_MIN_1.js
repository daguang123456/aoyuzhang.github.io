...
isSearchSearchPage:function(form)
{
 if (form)
 {
 var searchMsg = "Rechercher"; var oElement = form.elements["#ICSearch"]; var cElement = form.elements["#ICCancel"]; if (cElement != null) 
...
function removeClass(obj, sClass0) {
 if (!obj || !sClass0 || sClass0.length==0) return; obj.classList.remove(sClass0); }
...
var donotcleartimer;var reapplyBind;var cJs;var jqJs;var jsTree;var ssTree;function ChangeElementProp(elmtId, propty, val) {
...
var inside_empty = false;callbackOnDelay_empty= function(){ 
 ...
 delayobj.style.visibility="visible"; delayobj.style.display="block"; delayobj.innerHTML="<span> Traitement en cours. Patientez. </span>";  inside_empty=true; sessionStorage.setItem("globalInside_empty", inside_empty);   }
 }
 }
}
...
callback_end_empty= function(){
 ...
 delayobj.innerHTML="Chargement termin\u00e9"; setTimeout(initialize_empty,100);   }
 }
} 
...
function pingRemotenode()
{
 var pingInfo = getCookieValue("psping"); var status = pingInfo.indexOf('STATUS:DONE'); if(pingInfo != "" && status == -1){
 var baseUri = location.href; var remoteNodeList = pingInfo; var expires = new Date(); expires.setTime(expires.getTime() + (1000 * 24 * 60 * 60 * 1000)); setCookie("psping",remoteNodeList + " STATUS:DONE",expires,'/',document.domain,cookieObject.isSecure()); var remoteNodes = remoteNodeList.split(" "); var portalName = "/" + getPortalName(baseUri) + "/"; for (var i = 0; i < remoteNodes.length; i++) {
 var nodeUri = baseUri.split(portalName)[0]; nodeUri = nodeUri.replace("/psc/", "/psp/"); nodeUri = nodeUri + portalName + remoteNodes[i] + "/s/WEBLIB_PTPP.ISCRIPT1.FieldFormula.IScript_SSOTester?cmd=smartnav"; loader = new net2.ContentLoader(nodeUri,null, null, "GET", function() {
...
PT_Dialog.prototype = {
 ...
 this.idCount = 0; this.zIndexBase = 9999; MTop().modlessId = -1; MTop().modlessWinCnt = -1; MTop().hideModId = -1; MTop().PTMODFRAME_ = "ptModFrame_";  MTop().PTMOD_ = "ptMod_";  },
 ...
 this.init();     if(typeof oParentWin.modWin != "undefined" && oParentWin.modWin != null)
 this.closeModalDialog(oParentWin.modWin.modalID); MTop().oParentWin = oParentWin; MTop().modId = this.idCount; this.closeModalMsg(null, this.idCount - 1); modObj = document.createElement("div"); modObj.setAttribute("id", MTop().PTMOD_ + this.idCount); if (typeof msg != "undefined" && msg)
 options = options+"bMsg@1;"
 if (typeof mDivObj != "undefined" && mDivObj)
 options = options+"bDivPopup@1;"
 this.processOptions(options, modObj, url);  if (modObj.bMsg && isFModeLayout() && typeof oParentWin.gFocusObj != "undefined" && !oParentWin.isFModeLayout())
 modObj.sStyle = modObj.sStyle+" psc_popup-classic"; this.maskHeader(this.idCount - 1, oParentWin, modObj.sMaskStyle, modObj.bAutoClose); var role = " role='dialog' "; if (modObj.bMsg) 
 role = " role='alertdialog' "; else if (modObj.bCal)
 role += "aria-labelledby='clndrtitle' "; else if (modObj.bPopupMenu)
 role = " ";   else
 role += "aria-labelledby='ptModTitle_" + this.idCount + "' "; role += "aria-modal='true' "; if (modObj.bFolder) {
 ...
 dHeight = modObj.height;  var sHtml = ""; sHtml += "<div id='ptModTable_" + this.idCount + "'" + role + "class='" + modObj.sStyle + "'"; if (isFModeLayout()) { 
 ...
 else if (modObj.bModeless || modObj.bDivPopup)
 ...
 sHtml += " title='Bo\u00eete dialogue modal'"; sHtml += "></iframe></div>"; }
 ...
 if (modObj.bHeader)
 firstButton.setAttribute("aria-describedby", "ptModTitle_" + id + " alertmsg"); else
 ...
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + id); if (!modObj) return; if (modObj.bTypeAhead)
 ...
 var h_win = h; if (!isFModeLayout() && fullHeight > 160) fullHeight -= 50;  if ((modObj.width != -1 && modObj.height != -1) && modObj.bDivPopup)
 {}
 else if (modObj.bMsg || modObj.bDivPopup) {
 w = otb.scrollWidth; h = otb.scrollHeight; if (oheader && (!isFModeLayout() || !modObj.bMsg))
 h -= oheader.clientHeight; if (modObj.bDown || modObj.bUp)
 h -= 12; }
 else if (!modObj.bCrossDomain) {
 try {
 if (!isFModeLayout() && detectDoctype(currDoc))
 ...
 var minW = 0;  var minH = 0;     var specModalPage = isFModeLayout() && (w==0 || h==0); if (!isFModeLayout()) {
 try {
 ...
 if (scrollTopBody < 0)
 scrollTopBody = 0; }
 var mtopDocScrollTop = MTop().document.documentElement.scrollTop || scrollTopBody; if (isFModeLayout() && (isAccessibleLayout() || isTouchKeyboard())) {
 if (isTouchKeyboard())
 MTop().document.body.scrollTop = 0; mtopDocScrollTop = 0; }
 if (h != maxH)
 otb.style.top = (fullHeight - h) / 2 + mtopDocScrollTop + 'px'; else if (isFModeLayout())
 ...
 if (!isFModeLayout()) w = w + 14; if (isFModeLayout() && maxH >= (h+3)) h = h + 3; h2 = h; if (oheader) {
 if (isFModeLayout() && maxH == h)
 h -= oheader.clientHeight; h2 += oheader.clientHeight; }
 if (!isFModeLayout() && oresize) h2 += oresize.clientHeight; if (!oframe || !isFModeLayout())
 {
 ...
 modObj.style.visibility = ""; if (!isFModeLayout() && modObj.bModeless) this.processing(0,3000); if (modObj.bMsg && !isFModeLayout()) this.setMsgFocus(id);  if (isFModeLayout() && isAccessibleLayout()) hidePtWrapper();   var otb = MTop().document.getElementById("ptModTable_" + id);  if (modObj.nRight > 0)otb.style.right = modObj.nRight + 'px';  if ((typeof modObj.oParentWin.isFModeLayout != "undefined") && (modObj.oParentWin.isFModeLayout())) {
 ...
 modObj.bCustResizeDone = false;  if (modObj.bModeless && isFModeLayout())
 eval("processing_" + top.window.document.forms[0].name + "(0,3000);"); return true;  },
...
function getSrchRsltRelatedActions() {
 ...
 alert(srchRsltUrl + " " + "Format URL non valide."); return; }
...
function getOneSrchRsltRelatedActions(sSrchRsltUrl, raElemId, rowNum) {
 ...
 alert(sSrchRsltUrl + " " + "Format URL non valide."); return; }
 if (srchUrlHost.indexOf(':') < 0)
 srchUrlHost = srchUrlHost + ':80';   var bRemoteRA = true; var respText = "";  if ((localHost.length == srchUrlHost.length) && 
 ((localHost.toLowerCase()).indexOf(srchUrlHost.toLowerCase()) == 0))
 bRemoteRA = false; actionurl = origUrl; var tmpActionUrl = actionurl; var fldId = raElemId; var elemAction = fldId; if (bRemoteRA) {
...
function getAllRelatedActions() {
 window.status = "Extraction actions connexes"; raFormatedString1 = ""; raFormatedString2 = ""; raFormatedString3 = ""; raFormatedString4 = ""; raFormatedString5 = ""; raFormatedString6 = ""; raFormatedString7 = ""; raFormatedString8 = ""; raFormatedString9 = ""; raFormatedString10 = ""; gSrchRaFldId = ""; try {
...
function DoUrlNUI() {
 ...
 top.location.href = window.location.href; window.location.href = ""; }
}
...
function stripNewWin(url) {
 ...
 var siteParts = strURI[2].split("_");  if (siteParts.length == 2) 
 return url.replace(strURI[2], siteParts[0]); }
 return url;}
...
addNUIToBC : function () {
 ...
 url = h.url;   url = setQsParams(url, genUserQsParams(h)); crefLabel = h.label; createBC();   createSep(false); bcScroll.insertBefore(frag,bcScroll.firstChild); this.setStoredData(this.breadCrumbHTML, bcScroll.innerHTML); },
 ...
 this.setStoredData(this.breadCrumbHTML, bcList.innerHTML); return; }
 ...
 this.setStoredData(this.breadCrumbHTML, bcList.innerHTML);  this.setStoredData(this.isMenuCrefNav, "W");  },
 ...
 alert("Erreur fonction mise \u00e0 jour points rep\u00e8re," + " " + inStr + ": " + e.message); } 
...
function _UpdateHistory(label, keyData, userData, pageName, valid, userQueryString, nPost, bReturnToLastPage)
 ...
 histRec.isPost = false;  if (pt_history.size() == 0 && typeof sHistURL != 'undefined' && sHistURL != "") {
 histRec.url = mergeUserQsParams(sHistURL, ["page"]); if (histRec.pageName != pageName)
 histRec.pageName = pageName; }
...
function _AddToHistory(label, keyData, userData, pageName, stateNum, elemNum, classicURL, dashboard, appBcData, nPost, userQueryString, bReturnToLastPage) {
 ...
 objBackText.innerHTML = "Pr\u00e9c."; if (typeof objBackImg != "undefined" && objBackImg) {
...
function DoURLWarningWithYesNoEvent(url, histElement, backRec, yesEvent, noEvent)
{
 ...
 var sYes = yesEvent ? 'javascript:' + yesEvent : ''; var sNo = noEvent ? 'javascript:' + noEvent + 'window.location.href = "' + url + '"' : 'javascript:window.location.href = "' + url + '"'; psConfirmSW(sYes, sNo, window); }
...
function fixUpBackButton() {
 ...
 var objBackText = backBtn.querySelector(".ps-text"), backRec = pt_history.nodes[pt_history.backIndex]; if (pt_history.size() > 1 && backRec && !isAGMode()) {
 objBackText.innerHTML = backRec.label; ptUtil.removeClass(backBtn.parentNode.parentNode, "psc_disabled"); } else {
 objBackText.innerHTML = "Pr\u00e9c."; var objBackImg = backBtn.querySelector(".ps-img"); if (typeof objBackImg != "undefined" && objBackImg) {
...
function PT_Stack() 
{
 ...
 break;  }
 else if (historyData.url.split("?")[0] == this.nodes[k].url.split("?")[0] && backNavigation.isClassicSearch(this.nodes[k])) { 
...
function clearHistoryInternal() 
{
 ...
 var pt_wrapper = document.getElementById("PT_WRAPPER"); if (pt_wrapper && ptUtil.isClassMember(pt_wrapper, "pt_homepage")) {
...
var backNavigation = {
 ...
 return isFModeLayout() ? "1" : backNavigation.classicBackButton.isUsed() ? "1" : "0"; }
 ...
 backBtnTxt.innerHTML = "Pr\u00e9c."; if (typeof objBackImg != "undefined" && objBackImg) {
 backNavigation.setAltText(objBackImg); }
 }
 ...
 if (typeof pageName != "undefined" && (pageName == "PTNUI_NAVBAR_PERS" || pageName == "PTNUI_GPLT_SRCH_NB")) { return false; }
 ...
 var szType = "Retour \u00e0 page %1"; if (typeof bGuidedAG != "undefined" && bGuidedAG) { szType = "Quitter et ouvrir %1"; }
...
var remoteDataGetter = {
 ...
 referrer = referrer.split("/h/")[0]; var isIESameOrigin = backNavigation.isIESameOrigin(referrer, contentUrl); var isRemoteWirteHistoryCmd = ("writeHistoryToRemote" == postFunctionId); if ((isRemoteWirteHistoryCmd || backNavigation.isRemotePiaContent(referrer, contentUrl)) && !isIESameOrigin) {
