...
function ptPgltWarn(form, id, event) {
 ...
 return; var msgTitle = "Confirmation de sortie de pagelet"; var msg = "Vous \u00eates sur le point d\'acc\u00e9der \u00e0 une nouvelle page. Voulez-vous continuer?"; var evtYes = (typeof ptPgltTransWarn != "undefined") ? ("ptPgltTransWarn=false;") : ""; var evtPassForm = ((typeof form != "undefined") && (form != null)) ? "submitAction_" + form.name + "(document." + form.name + ",'" + encodeURI(id) + "');" : ""; var evtNo = "pgltWarningOff();closeMsg(this);"; if (typeof modalCloseUrl != 'undefined' && modalCloseUrl) {
 var sYes = "Oui"; var sNo = "Non";  var sChkBoxMsg = "D\u00e9sactiv. permanente avertiss. sortie de pagelet. Pour le r\u00e9activer : page de personnalisation."; shtml = "<div id='msgTitle' style='display:none;'>" + msgTitle + "</div>";  shtml += "<div id='alertmsg' tabindex='0' role='presentation'><span class='popupText' id='alerttxt'>" + msg + "</span>"; shtml += "<div> <input type='checkbox' onclick='if(this.checked == true)" + evtYes + "' class='PSCHECKBOX' id='PERSN_PGLT_WARN' >"; shtml +="<label class='PSCHECKBOX' id='PERSN_PGLT_WARN_LBL' for='PERSN_PGLT_WARN'>" + sChkBoxMsg + "</label></div></div>";  shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' ><span><input type=button id='ALERTOK' class='PSPUSHBUTTONTBOK' value='" + sYes + "'";  shtml += " alt='" + sYes + "' title='" + sYes + "' onclick=\"pgltWarningOff();closeMsg(this);" + evtYes + evtPassForm + "\"/></span></a>";  shtml += "<a class='PSPUSHBUTTON Left' ><span><input type=button id='ALERTCANCEL' class='PSPUSHBUTTONTBOK' value='" + sNo + "'";  shtml += " alt='" + sNo + "' title='" + sNo + "' onclick='" + evtNo + "' /></span></a></div>";   addMsg(shtml, window, '');  playMsg();    var gpModTable = document.getElementById('alertmsg');  if (gpModTable) {
...
if ((changed==null) && top.frames)
 ...
 return psConfirmFluid("Avert. enreg.", "Voulez-vous enregistrer vos modifications?" , "YesNo", saveOKEvent, saveCancelEvent, "Toutes les modifications des outils d\'orientation seront perdues. Oui pour enregistrer, Non pour annuler les modifications..", pWin, bDelay);}
var rtn = true;if (changed)
{
 ...
 if (typeof isNewSaveWarn !== "undefined" && isNewSaveWarn) {
  psConfirm2("Avert. enreg.", "Enregistrer vos modifications? Oui pour enregistrer, Non pour annuler les modifications.", "YesNo", saveOKEvent, saveCancelEvent, null, bDelay); } else {
  psConfirm2("Avert. enreg.", "Cliquez sur OK pour revenir \u00e0 la page et enregistrer vos donn\u00e9es ou sur Annuler pour continuer.", "OKCancel", saveCancelEvent, saveOKEvent, null, bDelay); }
 }
 else {
 if (typeof pWin == 'undefined' || pWin == null) { pWin = window; }
 return psConfirmFluid("Avert. enreg.", "Voulez-vous enregistrer vos modifications?" , "YesNo", saveOKEvent, saveCancelEvent, "Cliquez sur Oui pour retourner en arri\u00e8re et enregistrer et sur Non pour annuler les modifications.", pWin, bDelay); }

 return; } else
 rtn = !confirm("Cliquez sur OK pour revenir \u00e0 la page et enregistrer vos donn\u00e9es ou sur Annuler pour continuer.");}
...
function setupTimeout2(nTimeoutMilliseconds, nWarningTimeoutMilliseconds)
{
 ...
 timeoutID = window.setTimeout('displayTimeoutMsg2()', nTimeoutMilliseconds); if (isFModeLayout() || isTopFModeLayout())
...
function psConfirmSW(saveOKEvent, saveCancelEvent, pWin) {
 ...
 var agTarget = typeof document.getElementById("ICAGTarget") != 'undefined' && document.getElementById("ICAGTarget"); var agChanged = agTarget ? (typeof document.getElementById("ICUOW") != 'undefined' && document.getElementById("ICUOW") != null && document.getElementById("ICUOW").value != "N" ? true : false) : false; if (agChanged)
 return psConfirmFluid("Avert. enreg.", "Voulez-vous enregistrer vos modifications?" , "YesNo", saveOKEvent, saveCancelEvent, "Toutes les modifications des outils d\'orientation seront perdues. Oui pour enregistrer, Non pour annuler les modifications..", pWin); }

 if (!isFModeLayout() && !isTopFModeLayout()) {
 if (typeof isNewSaveWarn !== "undefined" && isNewSaveWarn) {
 return psConfirmClassic("Avert. enreg.", "Enregistrer vos modifications? Oui pour enregistrer, Non pour annuler les modifications.", "YesNo", saveOKEvent, saveCancelEvent, pWin); } else {
 return psConfirmClassic("Avert. enreg.", "Cliquez sur OK pour revenir \u00e0 la page et enregistrer vos donn\u00e9es ou sur Annuler pour continuer.", "OKCancel", saveCancelEvent, saveOKEvent, pWin); }
 }
 else {
 return psConfirmFluid("Avert. enreg.", "Voulez-vous enregistrer vos modifications?" , "YesNo", saveOKEvent, saveCancelEvent, "Cliquez sur Oui pour retourner en arri\u00e8re et enregistrer et sur Non pour annuler les modifications.", pWin); }
}
...
function psConfirm2(msgTitle, msg, msgType, event1, event2, pWin, bDelay) {
 ...
 sCancel = "Annuler",
 sYes = "Oui",
 sNo = "Non"; shtml = "<div id='ptpopup' class='popupText' role='alertdialog' aria-labelledby='msgTitle' aria-describedby='alertmsg'>"
 ...
 eval(event2); }
 if (msgType == "YesNo") {
 var rtn = !confirm("Cliquez sur OK pour revenir \u00e0 la page et enregistrer vos donn\u00e9es ou sur Annuler pour continuer."); if (rtn)
 eval(event2); else
 eval(event1); }
 }
}
...
function psConfirmClassic(msgTitle, msg, msgType, event1, event2, pWin) {
 ...
 sCancel = "Annuler",
 sYes = "Oui",
 sNo = "Non"; var shtml = "<div id='msgTitle' style='display:none;'>" + msgTitle + "</div>"; shtml += "<div id='alertmsg'><span class='popupText'>" + msg + "</span></div>"; if (msgType == "OKCancel") {
...
function psConfirmFluid(msgTitle, msg, msgType, event1, event2, longMsg, pWin, bDelay) {
 ...
 shtml += "<div id='#ICOK' class='ps_box-button psc_ok'><span id='#ICOK$span' class='ps-button-wrapper' title='Ok (Enter)'><a class='ps-button' id='#ALERTOK' role='button'"; shtml += " onclick='closeLastModal();" + event1 +"' href='javascript:void(0);'/><span class='ps-text'>" + sOK + "</span></a></span></div>"; } else {
